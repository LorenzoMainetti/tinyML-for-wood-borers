...
int main(void)
{
  ...
  uint8_t resultBuf[6000];  // TODO not needed
  uint8_t *pRes = &resultBuf[0];
  uint16_t background, bark, longhorn;
  ...
  while (1)
  {
	  ...
	  if (float_count == WIN_SIZE) {
		  ...
		  // 2. bandpass filter
		  iir_filter(signalBuffer, filteredSignal, WIN_SIZE);  // not window-dependent [OPTIMIZE]

		  print_duration(timer_val, 'F');  //filtering
		  timer_val = __HAL_TIM_GET_COUNTER(&htim16);

		  // 3. raw signal standardization
		  //standardize(filteredSignal, WIN_SIZE);   // window-dependent
		  // 4. std detector
		  detected = rolling_std_detector_v2(filteredSignal);

		  print_duration(timer_val, 'D');  //detection
		  timer_val = __HAL_TIM_GET_COUNTER(&htim16);

		  if (detected) {
			  HAL_GPIO_TogglePin(LED3_GPIO_Port, LED3_Pin);

			  // 5a. spectrogram extraction
			  spectrogram_run(filteredSignal, spectrogram);
			  // 6a. spectrogram standardization
			  standardize(spectrogram, SPECTROGRAM_ROWS * SPECTROGRAM_COLS);

			  print_duration(timer_val, 'S');  //spectrogram
			  timer_val = __HAL_TIM_GET_COUNTER(&htim16);

			  // 7a. cnn classifier
			  cnn_classifier(spectrogram, &classPrediction);
			  // 8a. output processing and visualization
			  if (classPrediction == 0) {
				  background += 1;
			  	  memcpy(pRes, "0", 1);
			  	  printf("[STM32] detected interference 0\n");
			  }
			  else if (classPrediction == 1) {
				  longhorn += 1;
				  memcpy(pRes, "1", 1);
				  printf("[STM32] detected insect sound: Longhorn Beetle 1\n");
			  }
			  else if (classPrediction == 2) {
				  bark += 1;
			  	  memcpy(pRes, "2", 1);
			  	  printf("[STM32] detected insect sound: Bark Beetle 2\n");
			  }
			  else {
				  printf("[STM32] Inference error\n");
			  }

			  print_duration(timer_val, 'C');  //classification
		  }
		  else {
			  // 5b. output visualization
			  HAL_GPIO_TogglePin(LED2_GPIO_Port, LED2_Pin);

			  background += 1;
			  memcpy(pRes, "0", 1);
			  printf("[STM32] No insect sound detected 0\n");
		  }
